...
ann_file = 'data/demo/simple_data.pkl'
...
train_pipeline = [
    ...
    dict(type='GeneratePoseTarget', with_kp=False, with_limb=True), # Generate pseudo heatmaps based on joint coordinates and confidence.
...
test_pipeline = [ # input shape [2*num_clips, num_joints, clip_len, resize_w, resize_h]
    ...
    dict(type='PoseCompact', hw_ratio=1., allow_imgpad=True),
    dict(type='Resize', scale=(64, 64), keep_ratio=False),
    dict(type='GeneratePoseTarget', with_kp=False, with_limb=True, double=True, left_kp=left_kp, right_kp=right_kp), # Generate pseudo heatmaps based on joint coordinates and confidence.
    dict(type='FormatShape', input_format='NCTHW_Heatmap'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['imgs'])
]
...
# optimizer
optimizer = dict(type='SGD', lr=0.01, momentum=0.9, weight_decay=0.0003)  # this lr is used for 8 gpus, finetue change 0.4 to 0.01
